  ...
  SurfReact(sparta, narg, arg)
{
  ...
  //VA - trying to delay surf_react adsorb being used until surface exists
...
SurfReactAdsorb::~SurfReactAdsorb()
{
  ...
  // in case multiple surf react/adsorb instances are used
    ...
    grid->remove_custom(area_custom); //remove volume_custom
    grid->remove_custom(weight_custom);
    if (psflag) surf->remove_custom(tau_custom);
  }

  // delete local face and per-surf data
    //VA - want to delete local grid data
  if (mode == FACE) {
...
void SurfReactAdsorb::create_per_surf_state()
{
  ...
  // VA - for implicit surfs, should be per-grid data instead? or leave as surf attributes, then later sum surf tallies to owning cells?
...
void SurfReactAdsorb::create_per_grid_state()
{
  ...
  // local delta storage
  // initialize surf and grid_species_delta for first time
  
  //VA - want to initialize both surf and  grid_species_delta?
  
  int nglocal = grid->nlocal; //VA - number of child cells--all 3 types-- I own
  memory->create(grid_species_delta,nglocal,nspecies_surf,
                 "react/adsorb:grid_species_delta");
  int nlocal = surf->nlocal;
  memory->create(surf_species_delta,nlocal,nspecies_surf,
                 "react/adsorb:surf_species_delta");
  ...
  //VA - zero grid_species_delta
   for (int icell = 0; icell < nglocal; icell++)
    for (int isp = 0; isp < nspecies_surf; isp++)
      grid_species_delta[icell][isp] = 0;

   // VA - previous surf-based attributes
  // set ptrs used by react() and PS_react() to per-surf data structs
  //species_state = surf_species_state;
  //total_state = surf_total_state;
  //area = surf_area;
  ...
  area = grid_area;
  weight = grid_weight;
  species_delta = grid_species_delta;
  
  // allocate data structs for periodic sync
  //VA - want grid->nown?
  int nown = surf->nown; //for implicit surfs, nown = nlocal= surfs in owned gridcells

  //VA - want to create the grid-equivalent of these arrays
  memory->create(mark,nlocal,"react/adsorb:mark"); // =1 if reaction occured on surf, else 0; mark[isurf]->mark[icell]?
  ...
  for (int isurf = 0; isurf < nlocal; isurf++) mark[isurf] = 0;
  //VA - mark[icell]? do I mark an entire grid cell when reaction takes place on a single triangle element?
}
...
void SurfReactAdsorb::init()
{
  ...
  if (domain->dimension == 2) {
    ...
    for (int isurf = 0; isurf < nlocal; isurf++) {
      ...
      //for implicit, distributed; nlocal = nown = surfs in owned grid cells

      //for each grid cell
         //for each CONTAINED tri
          //area[icell]= sum these tri areas 
      // if (psflag)
      //
      //}
      // }
...
// VA - perform surface reaction for particle in icell on surface element isurf??
...
int SurfReactAdsorb::react(Particle::OnePart *&ip, int isurf, double *norm,
                           Particle::OnePart *&jp) //VA - pass in icell
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    case AA:
      {         
        check_ads = 1;
        ads_index = i;
        double surf_cover = total_state[isurf] * ms_inv; //VA - total_state[icell]
        double S_theta = 0.0;
