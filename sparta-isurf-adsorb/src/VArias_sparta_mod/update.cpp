            ...
            if (cflag) {
              ...
              if (DIM == 3)
                jpart = surf->sc[tri->isc]->
                  collide(ipart,dtremain,minsurf,icell,tri->norm,tri->isr,reaction);
              if (DIM != 3)
                jpart = surf->sc[line->isc]->
                  collide(ipart,dtremain,minsurf,icell,line->norm,line->isr,reaction);
	...
	//   (c) cell on other side of face is a parent
	  ...
	  pcell = &pcells[neigh[outface]];
        ...
        // OTHER: reflected particle stays in same grid cell
          ...
          bflag = domain->collide(ipart,outface,0,xnew,dtremain,
                                  jpart,reaction);
