...
enum{FACE,SURF,ISURF}; //VA - adding ISURF
...
{
  // VA - remove surf->implicit condition
  ...
  else if (strcmp(arg[iarg+2],"isurf") == 0) mode = ISURF; // VA - adding implicit surface mode
  else error->all(FLERR,"Illegal surf_react adsorb command");

  
 // VA - condition is surf OR isurf mode and nsurf = 0
  if (mode == SURF || mode == ISURF && surf->nsurf == 0)
  ...
  // VA - if ps not used, nactive_ps isn't needed
  // gs can allocate tallies now
  if (!psflag && gsflag) {
    if (tallyflag == 0) {
      tallyflag = 1;
  ...
  if (mode == ISURF) create_per_grid_state(); //VA - for implicit surfs, create per grid data structs
}
...
SurfReactAdsorb::~SurfReactAdsorb()
{
  ...
  //VA - delete custom per-grid state data owned by GRID class
  // done exactly once by first_owner
  // in case multiple surf react/adsorb instances are used
    if (mode == ISURF && first_owner) {
    grid->remove_custom(nstick_species_custom);
    grid->remove_custom(nstick_total_custom);
    grid->remove_custom(area_custom);
    grid->remove_custom(weight_custom);
    if (psflag) grid->remove_custom(tau_custom);
  }

  // delete local face and per-surf data
    //VA - want to delete local per-grid data
  if (mode == FACE) {
...
void SurfReactAdsorb::create_per_surf_state()
{
  ...
  int nown = surf->nown; //for implicit surfs, nown = nlocal= surfs in owned gridcells 
...
/* ----------------------------------------------------------------------
   VA - create and initialize custom per-GRID data structs
   this must be done exactly once even if multiple SRA instances are used
------------------------------------------------------------------------- */

void SurfReactAdsorb::create_per_grid_state()
{
  // SRA instance with first_owner = 1 allocates the custom per-grid data
  // add_custom() intializes all state data to zero
  // custom indices are for any type of pergrid vec or array
  // direct indices are for specific types of vecs or arrays
  // cannot perform add_custom() for tau now, b/c nactive_ps is not yet known
  //   do this later in init

  // VA - for implicit surfs, grid creates and initializes state data 
  // want to add data structs eventually

  if (grid->find_custom((char *) "nstick") < 0) {
    first_owner = 1;
    nstick_species_custom = grid->add_custom((char *) "nstick_species",
					     INT,nspecies_surf);
    nstick_total_custom = grid->add_custom((char *) "nstick_total",INT,0);
    area_custom = grid->add_custom((char *) "area",DOUBLE,0); 
    weight_custom = grid->add_custom((char *) "weight",DOUBLE,0);
    if (psflag) tau_custom = -1;

  } else {
    first_owner = 0;
    nstick_species_custom = grid->find_custom((char *) "nstick_species");
    nstick_total_custom = grid->find_custom((char *) "nstick_total");
    area_custom = grid->find_custom((char *) "area");
    weight_custom = grid->find_custom((char *) "weight");
    if (psflag) tau_custom = grid->find_custom((char *) "tau");
  }

  //VA - want grid to point to index into each array/vec
  int nstick_species_direct = grid->ewhich[nstick_species_custom];
  int nstick_total_direct = grid->ewhich[nstick_total_custom];
  int area_direct = grid->ewhich[area_custom];
  int weight_direct = grid->ewhich[weight_custom];

  //VA - want per-grid data structs
  grid_species_state = grid->eiarray[nstick_species_direct];
  grid_total_state = grid->eivec[nstick_total_direct];
  grid_area = grid->edvec[area_direct];
  grid_weight = grid->edvec[weight_direct];

  // local delta storage
  // initialize surf_species_delta for first time
  
  //VA - want to initialize both surf and  grid_species_delta?
  // for now, grid_species_delta is not necessary
  
  int nlocal = surf->nlocal; //for implicit surfs, nlocal = surfs in owned grid cells 
  memory->create(surf_species_delta,nlocal,nspecies_surf,
                 "react/adsorb:surf_species_delta");
  
  //VA - keep surf_species_delta to later sum?
  for (int isurf = 0; isurf < nlocal; isurf++)
    for (int isp = 0; isp < nspecies_surf; isp++)
      surf_species_delta[isurf][isp] = 0;
  
  //VA - set ptrs used by react() and PS_react() to per-grid data structs
  species_state = grid_species_state;
  total_state = grid_total_state;
  area = grid_area;
  weight = grid_weight;
  species_delta = surf_species_delta; //remaining as per-grid data struct
  
  // allocate data structs for periodic sync
  int nown = surf->nown; //for implicit surfs, nown = nlocal= surfs in owned gridcells

  //VA - remaining as per-surf arrays
  memory->create(mark,nlocal,"react/adsorb:mark"); // =1 if reaction occured on surf, else 0;
  memory->create(tally2surf,nlocal,"react/adsorb:tally2surf"); //global surf index (1 to Nsurf) for each row in intally, outtally, and outcollate arrays
  memory->create(intally,nlocal,nspecies_surf,"react/adsorb:intally"); 
  memory->create(outtally,nlocal,nspecies_surf,"react/adsorb:outtally");
  memory->create(outcollate,nown,nspecies_surf,"react/adsorb:outcollate"); //values only for owned surfs
...
void SurfReactAdsorb::init()
{
  ...
  if (psflag) {
    ...
    } else if (mode == ISURF) { //VA - adding tau as grid struct
      if (tau_custom == -1)
	tau_custom = grid->add_custom((char *) "tau",DOUBLE,nactive_ps);
      int itau = grid->ewhich[tau_custom];
      tau = grid->edarray[itau];
    }
  }
  ...
  Grid::ChildCell *cells = grid->cells; //VA - adding cells
  ...
  //VA - need to change if logic to account for implicit in 2d
  if (domain->dimension == 2) {
    for (int isurf = 0; isurf < nlocal; isurf++) {
      isr = lines[isurf].isr;
  ...
  } else if (mode==SURF) { //VA - explicit surfs
    double tmp;
    //for distributed, explicit; nlocal = Nsurf/P = # of surfs belonging to this proc
    //for all, explicit; nlocal = Nsurf= each proc owns all surfs
    for (int isurf = 0; isurf < nlocal; isurf++) {
      isr = tris[isurf].isr; //this instance of surf react
      if (surf->sr[isr] != this) return;
      area[isurf] = surf->tri_size(&tris[isurf],tmp); //cylces through each tri to track its area
      weight[isurf] = 1.0;
      if (psflag)
        for (int isp = 0; isp < nactive_ps; isp++) tau[isurf][isp] = 0.0; //empty tau
    }
  } else if (mode==ISURF) { //VA - implicit surfs
    double tmp;
    for (int icell = 0; icell < nglocal; icell++) { //for each grid cell owned by this proc
      int nsurf = cells[icell].nsurf; //number of surfs in a cell
      surfint *csurfs = cells[icell].csurfs; //indicies of surfs in a cell
	for (int m = 0; m < nsurf; m++) { //for each surf in a cell
	  int isurf = csurfs[m]; //surf index for each surf in a cell
	  // area[isurf] = surf->tri_size(&tris[isurf],tmp);
	  // area[icell] += area[isurf];
	  area[icell] += surf->tri_size(&tris[isurf],tmp);//update area of cell by summing over areas of each surf in cell
	  weight[icell] = 1.0;
	//empty tau[icell]
	if (psflag)
	  for (int isp = 0; isp < nactive_ps; isp++) tau[icell][isp] = 0.0; 
	}
      }
  }
}
...
int SurfReactAdsorb::react(Particle::OnePart *&ip, int isurf, int icell, double *norm,
                           Particle::OnePart *&jp) //VA - passed in icell
{
  ...
  //VA - add errors to avoid conflicts with implicit surf elements
  ...
  double fnum = update->fnum;
  long int maxstick = ceil(max_cover*area[icell] / (fnum*weight[icell]));
  double factor = fnum * weight[icell] / area[icell];
  double ms_inv = factor / max_cover;
  ...
  for (int i = 0; i < n; i++) {
    ...
    case AA:
      {         
        //check_ads = 1;
        //ads_index = i;
        double surf_cover = total_state[icell] * ms_inv; //VA - previously total_state[isurf]
        double S_theta = 0.0;

        if (r->kisliuk_flag) //only invoked when kisliuk keyword is used in reaction file
        {
    ...
    case DA:
      {  
        double surf_cover = total_state[icell] * ms_inv;
        double S_theta = 0.0;
    ...
    case LH3:
      {
        double surf_cover = total_state[icell] * ms_inv;
        double S_theta = 0.0;
	//VA - printing surf_cover
	//	cout << "surface coverage is: " << surf_cover << endl;
    ...
    case ER: //VA - ER reaction will cause trouble because prob_value takes in total_state[isurf], so make sure to not include in gs reaction file (OR use constant definition of total_state = surf_cover/ms_inv)
      {         
	    ...
	    (maxstick - total_state[icell]) * ms_inv / fabs(dot);
	    //(maxstick * ms_inv - surf_cover) / fabs(dot);
        } else {
    ...
    //VA - assuming full surface coverage, total_state (total ads count at last sync) will be constant/uniform across all surfs, so total_state = surf_cover*max_cover (or surf_cover/ms_inv)
    for (int j = 1; j < r->nreactant; j++) { //for each reactant
      if (r->state_reactants[j][0] == 's') { //if reactant is surface state
        if (r->part_reactants[j] == 0) {  //and if reactant is a catalyst?
          prob_value[i] *=  //update reaction probability using stoich powers of total_state and ms_inv
            stoich_pow(total_state[icell],
	    //stoich_pow((surf_cover/ms_inv),
                       r->stoich_reactants[j]) * 
            pow(ms_inv,r->stoich_reactants[j]);
        } else {      //if reactant is not a catalyst?
          prob_value[i] *= //update reaction probability using stoich powers of species_state (perspecies ads count at last sync) and ms_inv for that particular reactant
            stoich_pow(species_state[icell][r->reactants_ad_index[j]], //leave as is?
                       r->stoich_reactants[j]) * 
            pow(ms_inv,r->stoich_reactants[j]);
        }
      }
...
void SurfReactAdsorb::tally_update()
{
  ...
  if (psflag) {
    ...
    else if (mode == SURF || mode == ISURF) update_state_surf();
    //else if (mode == ISURF) update_state_grid();
...
void SurfReactAdsorb::PS_chemistry()
{
  ...
  // VA - for implicit surf elements: each proc updates every Pth cell and surf it owns
  ...
  if (mode == FACE) {
    ...
    //int nsurf = surf->nsurf;
    //int ncell = grid->ncell; //global count
    //want nglocal & nlocal
    int nlocal = surf->nlocal;
    int nglocal = grid->nlocal;
    if (domain->dimension == 2) {
      for (int n = me; n < nglocal; n += nprocs)
	for (int m = me; m < nlocal; m += nprocs)
	  if (surf->lines[m].isr == this_index)
	    PS_react(PSLINE,m,n,surf->lines[m].norm);
    } else {
      for (int n = me; n < nglocal; n += nprocs)
	for (int m = me; m < nlocal; m += nprocs)
	  if (surf->tris[m].isr == this_index)
	    PS_react(PSTRI,m,n,surf->tris[m].norm);
    }
  }

  
  //VA - can't use MPI_Allreduce here?
  ...
  for (int i = 0; i < nall; i++) {
    ...
    icell = grid->id_find_child(0,0,boxlo,boxhi,x); //child cell only okay for implicit surfs?
    if (icell < 0) continue;
...
void SurfReactAdsorb::update_state_surf()
{
  ...
  Grid::ChildCell *cells = grid->cells; //VA
  Surf::Line *lines = surf->lines;
  Surf::Tri *tris = surf->tris;
  int nlocal = surf->nlocal;
  int nglocal = grid->nlocal;
  ...
  //VA - not performing collate for now
  //tallying must be handled differently for distributed surfs
  //i.e. each proc would only need to know the state of its owned surfs?
  //then cycling through each processor, sum tallies?
  ...
  //not performing MPI_Allreduce, see reasoning above ^^
  ...
  } else if (mode ==ISURF) { //VA - added implicit
    for (int icell = 0; icell < nglocal; i++) {
      total_state[icell] = 0;
      int nsurf = cells[icell].nsurf; //number of contained surfs
      surfint *csurfs = cells[icell].csurfs; //indicies of contained surfs
      for (int m = 0; m < nsurf; m++) { //for each contained surf
	int esurf = csurfs[m]; //surf index for each surf in a cell
	for (j = 0; j < nspecies_surf; j++) { //for each species type
	  species_state[icell][j] += incollate[esurf][j]; //new perspecies grid state old perspecies grid state + summed delta (normally outally, incollate for now), will be fully updated after cycling through every contained surf
	  species_state[icell][j] = MAX(0,species_state[icell][j]); //don't want counts < 0
	  total_state[icell] += species_state[icell][j]; //total grid state count is sum over species_state
	}
      }
    }
  }
 ...
 void SurfReactAdsorb::PS_react(int modePS, int isurf, double *norm)
{
 ...
 void SurfReactAdsorb::PS_react(int modePS, int isurf, int icell, double *norm)
 {
   // mark this surface element since performing on-surf chemistry
  ...
  for (int i = 0; i < nactive_ps; i++) {
    ...
    for (int j=0; j<r->nreactant; j++) {
      if (r->state_reactants[j][0] == 's') {
        if (species_state[icell][r->reactants_ad_index[j]] < r->stoich_reactants[j]) rxn_occur[i] = 0;
      }
    }
    //if (rxn_occur[i]) tau[isurf][react_num] += update->dt;
    //if (rxn_occur[i]) tau[isurf][i] += update->dt*nsync;
    if (rxn_occur[i]) tau[icell][i] += update->dt*nsync;
  }
      ...
      if (rxn_occur[i]) {
        ...
        if (r->type == SB) {
          ...
          if (r->state_reactants[j][0] == 's') { 
            factor_pow += r->stoich_reactants[j];            
            if (r->part_reactants[j] == 0) { 
              nu_react[i] *= stoich_pow(total_state[icell],r->stoich_reactants[j]);   
            } else {
              nu_react[i] *= stoich_pow(species_state[icell][r->reactants_ad_index[j]],r->stoich_reactants[j]);
            }            
          }
        }
        ...
        */
        ...
        //nu_tau[i] = MAX(floor(nu_react[i] * tau[isurf][react_num]),0);
        //nu_tau[i] = MAX(floor(nu_react[i] * tau[isurf][i]),0);
	nu_tau[i] = MAX(floor(nu_react[i] * tau[icell][i]),0);
        sum_nu_tau += nu_tau[i]; 
      }
    }
