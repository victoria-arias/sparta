...
class SurfReactAdsorb : public SurfReact {
 public:
  ...
  ~SurfReactAdsorb();
  void init();
  int react(Particle::OnePart *&, int, int, double *, Particle::OnePart *&);
 ...
 private:
  ...
  int mode;                         // FACE or SURF or ISURF
  ...
  int **grid_species_state;     // ptrs to custom state vecs/arrays in Surf class
  int *grid_total_state;
  double *grid_area;
  double *grid_weight;
  //grid_tau?
 ...
 struct OneReaction_PS {
  ...
  void PS_react(int, int, int, double *); //overload
