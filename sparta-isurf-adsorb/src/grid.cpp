...
Grid::Grid(SPARTA *sparta) : Pointers(sparta)
{
  ...
  //VA - zero custom per-grid vectors/arrays
  ngcustom = 0;
  ename = NULL;
...
//VA - same as Surf::find_custom()                           
   ------------------------------------------------------------------------- */

int Grid::find_custom(char *name)
{
  for (int i = 0; i < ngcustom; i++)
...
int Grid::add_custom(char *name, int type, int size)
{
  ...
  for (index = 0; index < ngcustom; index++)
    if (ename[index] == NULL) break;

  if (index == ngcustom) {
    ngcustom++;
    ename = (char **) memory->srealloc(ename,ngcustom*sizeof(char *),
				       "grid:ename");
    memory->grow(etype,ngcustom,"grid:etype");
    memory->grow(esize,ngcustom,"grid:etype");
    memory->grow(ewhich,ngcustom,"grid:etype");
  }
...
/* ----------------------------------------------------------------------      
   ...
   //VA - same as Surf::allocate_custom()                                        
   ------------------------------------------------------------------------- */
